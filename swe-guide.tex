...
% Custom ellipsis spacing
\newcommand\mydots{\makebox[4em][c]{$\cdot$\hfil$\cdot$\hfil$\cdot$}}
...
% Five reasons to read this book: 1. It is a work of passion, born out of a consuming and frenetic obsession 2. It is painstakingly crafted, every sentence carefully constructed, every word meticulously chosen, 3. It is a work born from a deep depression that serves as a guide to those who may experience the same feelings of self-doubt, 4. It is a distillation of some very difficult and important ideas that you have encountered and will undoubtedly encounter again, ...
...
Say you want to \textit{communicate} an idea $X$ to someone. That is, you would like to convey some \textit{information} to them so that you might together establish a shared body of \textit{knowledge}. $X$ is on the tip of your tongue, but you cannot quite put it into words. Accordingly, you express a related idea $Y$, hoping that you might be able to suss out your target $X$ with the help of your fellow interlocutor. Your proxy idea $Y$ is either too specific or too general, a \textit{subclass} or \textit{superclass} of the \textit{class} $X$, and you must replace it with another idea, preferably $X$ or at least something closer to it. The conversation could go as follows: \\\\

\begin{addmargin}[8em]{2em}
\begin{flushright}
"Dude, what's that thing with the moving parts that beeps sometimes? It, like, gets hot if it moves too fast? \\[2mm]

\mydots \\[2mm]

It's like a \textit{machine} of some sort, but more \textit{specific}." \\[2\baselineskip]
\end{flushright}
\end{addmargin}

\begin{addmargin}[2em]{8em}
"Oh, yeah, those things with the engine in the front and the four doors? And it has two rows of seats, right? That's a \textit{sedan}." \\[2mm]
\end{addmargin}

\begin{addmargin}[8em]{2em}
	\begin{flushright}
		"No, not a sedan. It's more general than that." \\[2\baselineskip]
	\end{flushright}
\end{addmargin}

In this case, blah blah is a subclass of blah. \\

% Is type/category really necessary? Category probably should not be used this way. Maybe do types and classes.

A class is essentially a collection of \textit{things}, and any particular one of those things is called an \textit{instance} of that class. A subclass $Y$ defines a \textit{type} of $X$, and instances of $Y$ are said to be \textit{of} type $Y$ (e.g. \textit{Sedan} specifies a type of \textit{Car}, and \textit{sedans} are of type \textit{Sedan}). Similarly, a superclass $Y$ defines a \textit{category} that $X$ is a part of, and instances of $Y$ \textit{belong to} that category (e.g. \textit{Vehicle} is a category that \textit{Car} is a part of, and \textit{vehicles} belong to the category of \textit{Vehicle}). \\

Note, however, that the words \textit{type} and \textit{category} describe the same concept: a \textit{class}, a collection of things. The distinction between the two is \textit{relational} in nature. For example, while a \textit{Sedan} is a type of \textit{Car}, it may also be a category that subclasses like \textit{Toyota Camry} and \textit{Honda Civic} belong to. Likewise, \textit{Vehicle} is a category that \textit{Car} belongs to, but it may also be a type of \textit{Machine} or \textit{Consumer Good}. Types are \textit{generalized} and categories are \textit{specified} toward the target class $X$. \\

% FSM here

The process of \textit{generalization} is a boon to experts and a bane to amateurs. It is the unification of multiple types into a single category, a one from many, \textit{e pluribus unum}. The opposite concept, the partition of a single category into multiple types, many from one, \textit{ex uno plura}, is the process of \textit{specification}, which tends to be the easier of the two to understand. On the other hand, generalization tends to produce a richer and more detailed understanding. \\
...
% Top type is to universe/DoD as bottom type is to empty class.
...
% What does information mean to you, the reader? Keep that in mind as we discuss all this seemingly irrelevent stuff.
...
% A mathematical object in something that exists in a mathematical universe.
...
% It's easy to use a word that is close but ultimately incorrect.
...
% I also intend to continue my studies independently. I'm interested in delving deeper into machine learning, artificial intelligence, abstract algebra, modern logic, control systems, and automata theory. I may end up writing new auxiliary parts on those topics.
