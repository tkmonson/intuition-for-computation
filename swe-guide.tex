...
Much work was done in the 1930s to formalize "effective calculability". The results established \textit{Turing computability} as the correct formalization which led to the most significant definition in computability theory: any function whose outputs can be calculated with an algorithm is a \textit{computable function}. Computable functions are precisely those that can be computed by a Turing machine. We will discuss the formalization of computability and computable functions later in this section. For know, note that the terms \textit{computable} and \textit{solvable} can be used interchangeably to describe both functions and problems. \textit{Decidable} essentially means the same thing, but it is more appropriate for describing decisions and decision problems. \\

We will now discuss the history preceding and the circumstances of this foundational work in computability theory. Much of this work was done by Alonzo Church and Alan Turing in the 1930s, but their work built heavily on set-theoretical concepts discovered by German mathematician Georg Cantor in the late 19th century. After a crash course in Cantorian set theory, we will formalize the concepts of computability and computable functions before discussing their implications via the Church-Turing thesis. Afterward, we will briefly discuss uncomputable problems before moving into computational complexity theory, which classifies computable problems based on how resource-intensive they are. \\
...
\begin{center}
\begin{tcolorbox}[breakable,colback=white,width=12cm,boxrule=0pt,frame hidden]
...
We can then consider the power set of $S$, $\mathcal{P}(S)=\{\{\varepsilon\},\,\{0\},\,\{1\},\,\{0,\,1\},\,\cdots\}$ to be the set of all \textit{languages} that can be created using these binary words. A language $L$ is simply a subset of the set of all possible words over a finite alphabet. Stated formally, $L\subseteq\Sigma^*$. As previous discussed, languages can conform to a grammar, but for now we will consider them simply as sets of words. Words that belong to a given language are called \textit{well-formed words} and those that do not are called \textit{ill-formed words}, but only in relation to that language. \\
...
We know that Turing machines recognize recursively enumerable languages, so we know there are countably many of these. On the other hand, there are uncountably many \textit{non-recursively enumerable languages} that a Turing machine cannot recognize. In fact, almost all languages cannot be recognized by a Turing machine. \\
...
\begin{center}
	\begin{tcolorbox}[breakable,colback=white,width=12cm,boxrule=0pt,sharp corners]
