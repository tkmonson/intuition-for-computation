...
% This is the only section I didn't write notes for. I had my ideas, but I knew that, in the end, I would remember those who I truly wanted to thank and why I wanted to do so. Some things are better thought through with the heart than with the brain.

% DA teachers
% Duke teachers
% My good friends, those few people with whom it's never been anything less than \textit{real}, honest and earnest to infinity and beyond.
% Parents
% TeX, TeXStudio, Donald Knuth
% Hi-Chews, Seinfeld (Theory of Comedy, comedy systems, categories of comedic timing)
...
\begin{flushright}
"Dude, what's that thing with the moving parts that beeps sometimes? And it, like, gets hot if it moves too fast? \\[2mm]
...
"Oh yeah, those things with the engine in the front and the four doors? And it has two rows of seats, right? That's a \textit{Sedan}." \\[\baselineskip]
\end{addmargin}

\begin{addmargin}[8em]{2em}
	\begin{flushright}
		"No, not a sedan. It's more \textit{general} than that. But it does transport people and objects. It's definitely a type of \textit{Vehicle}." \\[2\baselineskip]
	\end{flushright}
\end{addmargin}

\begin{addmargin}[2em]{8em}
	"A \textit{Car}?" \\[\baselineskip]
\end{addmargin}

\begin{addmargin}[8em]{2em}
	\begin{flushright}
		"Oh, that's right, I'm thinking of a \textit{Car}. Ok, now that we're on the same page, remember how you lent me \textit{your car}? \\
		My bad, man, I crashed it." \\[2\baselineskip]
	\end{flushright}
\end{addmargin}

% A class is essentially a collection of \textit{things}, and any particular one of those things is called an \textit{instance} of that class.
% Here Interlocuter 1 presents Y_1... Specify, generalize, specify
...
Interlocutor $\mathcal{B}$ responds with \textit{specification}, adding assumed details to the definition of $Y_0$. Thus, $Y_1$ is proposed: the class \textit{Sedan}. Luckily, the assumptions are not totally off-base; $Y_1$ turns out to be a \textit{subclass} of $X$. $\mathcal{A}$ confirms this and so returns with a \textit{generalization} that conjures $Y_2$, the class \textit{Vehicle}. $\mathcal{B}$ specifies again, offering up as $Y_3$ a subclass of \textit{Vehicle} called \textit{Car}. Finally, $\mathcal{A}$ accepts this class as the target $X$ and ends the search. And with this mutual knowledge of \textit{Car} established, $\mathcal{A}$ then goes on to specify an \textit{instance} of \textit{Car}, a particular \textit{car} that belongs to $\mathcal{B}$. And with both interlocutors now cognizant of exactly what is being discussed, $\mathcal{A}$ swiftly informs $\mathcal{B}$ that his ride is totaled. \\

% FSM here?

% A and B generalize and specify their way to an answer.
The process of \textit{generalization} is a boon to experts and a bane to amateurs. It is the unification of multiple types into a single category, a one from many, \textit{e pluribus unum}. The opposite concept, partitioning a single category into multiple types, many from one, \textit{ex uno plura}, is the process of \textit{specification}, which tends to be the easier of the two to understand. On the other hand, generalization tends to produce a richer and more detailed understanding. \\
