...
I find it difficult to describe what this book is. Yet, I find it easy to express why I think it is worth your time and contemplation.

% https://www.standoutbooks.com/nail-non-fiction-introduction/

% The hook starts here. Why should the reader care?
% Five reasons to read this book: 1. It is a work of passion, born out of a consuming and frenetic obsession 2. It is painstakingly crafted, every sentence carefully constructed, every word meticulously chosen, 3. It is a distillation of some very difficult and important ideas that you have encountered and will undoubtedly encounter again, ...
...
The Ancient Greek poet Hesiod (fl. 750 BCE) depicts a fascinating, primordial uncertainty in his conception of Chaos: a vast nothingness that preceded the creation of the Universe. From this void emerged Gaia, Tartarus and Eros, personifying the Earth, the abyss beneath it, and love, respectively. Curiously, the modern definition of chaos is nearly inverted. Now, one might say that a situation is \textit{chaotic} if there is so much going on that it is hard to make sense of anything. Despite their great conceptual difference, these meanings share a common property: they both evoke \textit{confusion}. \\
...
A common approach to this challenge is to create a system of \textit{categories} that partition reality at the highest level. For example, one could suggest that there is a fundamental difference between a \textit{quantity} and a \textit{quality}. If these are considered distinct categories, we can classify \textit{measurable} or \textit{countable} stuff like \textit{length}, \textit{mass}, a \textit{dozen}, and a \textit{crowd} as quantitative and \textit{experiential} stuff like \textit{softness}, \textit{roundness}, \textit{cherry-flavor}, and \textit{beauty} as qualitative. \\
...
This curiosity aside, we could also design our category system such that quantities and qualities are considered \textit{properties} of some very general sort of thing. Philosophers have long considered the idea of a highest-level category and have ascribed a variety of names to its members: \textit{entity}, \textit{being}, \textit{thing}, \textit{term}, \textit{individual}, etc. Each carries its own subtle connotations. We will use the name \textit{object} because it is the default choice in current philosophical practice and because it has bled into the technical language of computer science with its meaning largely unchanged. \\

How general is this category of objects? Namely, are properties objects? If they are, then our system collapses to a single category, which really means that no distinctions about reality can be made at all. The category of objects thus becomes the soupy mess we are trying to escape. For this reason, let us assume that properties are \textit{not} objects. Instead, they are something else, and they \textit{relate} to objects in some way, giving them their characteristics. From this relationship, the diversity of existence unfolds. \\

%----------------------------------------

\subsection{Platonic and Aristotelian Metaphysics}

Plato (c. 428/427 -- c. 348/347 BCE) was the first to expound a metaphysics of objects with his \textit{theory of Forms}, which proposed a solution to the \textit{problem of universals}. This problem is subtle, but important: how can a single general property emerge in many particular objects? For example, when we say that a floor and a wall are \textit{smooth}, are we referring to some singular paradigm of \textit{smoothness} that is independent of each? For Plato, \textit{universals} or \textit{properties}, such as smoothness, are indeed separate from the \textit{particulars} or \textit{objects} that partake of them. He posits that the \textit{essence} of an object, its \textit{sine qua non}, is the product of its partaking of Forms. \\

Forms, as conceptualized by Plato, are abstract entities that exist in the \textit{hyperouranos}, the realm beyond the heavens. In the times of Homer and Hesiod, the Ancient Greek understanding of the cosmos closely mirrored the depiction given in the Hebrew Bible. The Earth was envisioned as a flat disc surrounded by endless ocean and the sky was a metal dome with gem-like stars embedded in it, much as it all appears from a naive perspective on the ground. Plato, however, was influenced by the philosophy of Pythagoras (c. 570 -- c. 495 BCE), who championed a spherical Earth for aesthetic reasons. Thus, he instead modeled the Earth as a "round body in the center of the heavens," and he imagined the Forms as part of a different, eternal world that transcends physical space and time. Whereas physical reality is the domain of opinion and raw sensation, the realm of the Forms is, as he paints it in \textit{Phaedrus}, the domain of \textit{reason}: \\
...
% The changing world can only be understood by the unchanging world. The multiple of the One.

% A demiurge or artisan instantiates the forms as particulars (Plato's attempt to reconcile his metaphysics with Hesiod's Theogony)
% The human soul becomes familiar with Form in heaven before being born into a body in the material plane
% You can only know Forms, not particulars (anti-realism?)
% Forms are "more real" than particulars
% Being, partaking, instantiation
% Lines blur between objects and properties. Forms represent properties, but they are also abstract objects. Particulars are objects, but their essense is a set of properties.
...
% Substantial vs. formal phenomena
...
% Models can be categorized: linear/non, static/dynamic, explicit/implicit, discrete/continuous, deterministic/probabilistic, deductive/inductive/floating
...
% We're not going to discuss religion or any theory that says more about the supernatural than the natural. However, we will briefly touch on some ancient models of physical reality that are intertwined with spirituality.

\subsubsection{Metaphysical Naturalism}
...
\subsection{Physics in Terms of Information}
...
% Computation with analog data means you are using actual, original, non-replicable quantites. Computation with digital data means you are using simulated quantities that are replicas or model of what you actually care about. We model EVERYTHING with numbers: physics, language, logic, images, video, music. Since we're going to be talking about digital data, we need to talk about numbers first.
...
\subsubsection{Euclid's \textit{Elements} and Mathematical Realism}
...
% Aristotle's logic was two-valued. However, he did not fully accept the low of excluded middle.

% Plato's doctrines were imprecise. Thus, Plato's theory of Forms is a reconstruction made by later scholars.
% Universals (Forms) vs material particulars
% Particulars "partake" of forms
% Being and Partaking: Forms are perfect, particulars approximate
% Each form is of one essence, and each essence is the essence of one form
% The only property of a Form is itself: Beauty is beautiful and nothing else.
% The physical world is full of copies of Forms
% Forms are immaterial, non-spatial, and atemporal. They do not change.
...
% A complex is a "whole made up of parts." A complex number is made up of two parts: one real part and one imaginary part.

\subsubsection{Numeral Systems}
...
% Most authority figures know little more than you do. Place your trust in outside ideas carefully. Get multiple opinions and come to your own conclusion. Otherwise, you'll be as clueless as they are.
...
% Thanks to Wikipedia, math.stackexchange, cs.stackexchange, philosophy.stackexchange, tex.stackexchange, Stack Overflow, Encyclopedia of Mathematics, Stanford Encyclopedia of Philosophy. I'm not going to reference every single page I used from these places. I used quite literally thousands of different web pages from these sites.
