...
Data are symbolic values that describe something of interest. In the case of quantitative data, these values are numbers that are \textit{encoded} in a numeral system of some \textit{base} or \textit{radix} that specifies how many unique \textit{symbols} or \textit{digits} are used to represent numbers. Data intended to be read by humans are typically encoded in a base-10 system, one of ten digits. \\
...
The base-60 system of Babylonian numerals is also notable. Its base is thought to have been chosen for its many factors, which simplify the process of division. Under this interpretation, this system is \textit{constructed} rather than natural. Instead of reflecting something concrete like physical extremities, this system chooses an abstract base in order to make an optimization. The reasoning for the use of base-2 encoding in computing is similar. It sacrifices human familiarity for performance and reliability. \\\\

\begin{tcolorbox}[breakable, enhanced, colback=textbook-blue, sharp corners]
	\vspace{2mm}
	\begin{center}
		\textbf{The Logic of Binary Encoding}
	\end{center}
	\vspace{1mm}
	% Implementation details: signals, logic levels, noise, power, reliability
	% Close relationship with classical, two-valued logic
	
	Binary encoding is not a requirement for computation. Rather, it is a concession that is made toward digital electronics. Computers have been designed for other bases, and, in some cases, a non-binary encoding is actually preferable. \\
	
	% Binary implies discrete, which is opposite of continuous
	% Discrete / continuous is independent of the implementation
...
\subsection{Automata, Calculators, and Computers}
...
\subsubsection{Formalization of an Abstract Machine}
