...
% Thanks to Wikipedia, math.stackexchange, cs.stackexchange, tex.stackexchange, Stack Overflow, Encyclopedia of Mathematics
...
\addcontentsline{toc}{subsection}{Data and Information}

Data are unorganized facts or statistical values that may reveal something of interest once structured and put into context. They are collected, cleaned, organized, and then analyzed in the hope of producing meaningful \textit{information}. Data can come in various forms, but \textit{quantitative data} are \textit{numbers} that describe \textit{quantity}. These data are collected via \textit{measurement}, a process that assigns a number to a quantifiable property that is being empirically observed. \\

Data are the building blocks of \textit{signals}, which are functions that model physical quantities that change over time. Signals map a set of times $t$ to a set of amplitudes $A$ and are classified according to the properties of these sets. The structure of $t$ determines whether a signal is \textit{continuous} or \textit{discrete}, and the structure of $A$ determines whether a signal is \textit{analog} or \textit{digital}. \\

The continuity of signals is similar to the continuity of mathematical functions, but the definitions differ slightly. Two properties of continuous functions are given below:

\begin{itemize}
	\item A function is \textit{continuous} over its domain if sufficiently small changes in input result in arbitrarily small changes in output.
	\item  Continuous functions are a subset of the real functions $f:D\rightarrow\mathbb{R}$ where $D$ is either $\mathbb{R}$ or an interval of it. In other words, a continuous function $f$ maps a \textit{linear continuum} $D$ to the real number line $\mathbb{R}$.
\end{itemize}

This first property is necessary and sufficient for defining continuous functions, and it implies the second property, which is necessary, but not sufficient. However, confusingly, a signal is considered continuous even if it satisfies the second and not the first. Thus, a signal is continuous if its time $t$ is a linear continuum and its amplitude $A$ is a subset of real numbers. Otherwise, it is \textit{discrete}. \\

Thanks to their real codomain, continuous signals have an infinite amount of possible output values. Any signal with this property is called \textit{analog} because an infinite-valued output can be seen as \textit{analogous} to the seemingly infinite complexity of the physical world. At any instant in its time domain, an analog signal can \textit{exactly} model a time-varying, real-world quantity, such as voltage, pressure, distance, mass, color, or anything else that is quantifiable. All continuous signals are analog, but not all analog signals are continuous. For example, the analog data of a continuous signal can be sampled at a finite rate in order to construct a discrete, analog signal. \\
...
\begin{center}
	\begin{tikzpicture}
		...
		\addplot [mark=none,
		          ...
		          textbook-blue-dark,
		          ultra thick,
		          restrict x to domain=0:12] {5*sin(deg(x))};
		\end{axis}
...
In general, signals can also be viewed as messages encoded in \textit{data} whose decoded content is \textit{information}. Like traditional written messages, ... Any signal can be expressed as an ordered sequence of data, each of which can be expressed as a tuple $(t,A(t))$. Because there are an infinite number of "instants" in any given interval of continuous time, the continuous signal shown above can be expressed an \textit{infinite}, ordered sequence of analog data. \\
...
\addcontentsline{toc}{subsection}{Language is Code}
...
\addcontentsline{toc}{subsection}{The Syntax and Semantics of Argument}
...
\addcontentsline{toc}{subsection}{Early Calculators and Computers}
