...
\textit{Reasoning} is a cognitive act performed by rational beings. It involves the absorption and synthesis of presently available information for the purpose of elucidating knowledge. It may require the ingenuity of thought, but in some cases the rote application of simple rules is sufficient. Reasoning could also be described as the providing of good \textit{reasons} to explain or justify things. However, as you might expect, there is much debate over whether or not any particular reason is "good" and whether or not it is a suitable justification for something. Reasoning is broad, and it comes in different flavors, but ultimately it is the pursuit of \textit{truth}. \\

%----------------------------------------

\toclineskip
\subsection*{Argument and Proof}

Reasoning may involve the application of \textit{logic}. This \textit{logical reasoning} is the kind of reasoning that can be expressed in the form of an \textit{argument}. However, this argument need not be \textit{valid}. In fact, it does not have to be a "good" argument at all. Such reasoning requires only that the steps involved be \textit{explainable} in some way, whether that be through informal speech or in formal writing. \\

The basic form of an argument like this has three main pieces: a set of \textit{premises}, a \textit{conclusion}, and a \textit{rule of inference}. A rule of inference can be thought of as a kind of logical function. That is, it takes premises as input and outputs a conclusion. A simple, one-step argument using such a rule might look like this: \\[2mm]
...
Inference is also called \textit{entailment}. Though these terms describe the same logical concept, the latter more aptly describes the relationship between statements that are linked. To infer is to come to a conclusion that is not \textit{explicitly} stated in the premises. Rather, the conclusion is \textit{deduced} from the premises. On the other hand, to entail is to require that something follows. If $P$ \textit{entails} $Q$, this suggests that $Q$ is a \textit{logical consequence} of $P$, which we can express using a \textit{turnstile}: $P\vdash Q$. Inference is something that fallible humans do. Entailment is something that logical rules do. \\

When inference matches entailment, you've got yourself a \textit{valid} argument. However, this does not necessarily mean that your argument is \textit{sound}. \textit{Validity} is a property of arguments whose conclusions must be true if their premises are true. An argument is valid as long as its conclusion is \textit{inferred} correctly from its premises (i.e. according to rules of inference that have been agreed upon). However, validity says nothing about whether or not the premises have a meaning that accurately describes reality. \textit{Soundness}, on the other hand, is a property of valid arguments whose premises are known to be true. \\

Validity and soundness complicate entailment. Now, an argument can be evaluated either in terms of \textit{syntax} (the text of the argument, either in symbols like $P$ and $Q$ or in a natural language) or in terms of \textit{semantics} (the actual meaning represented by the text). For example, consider the following valid use of \textit{modus ponens}: \\

\begin{center}
	\begin{tabular*}{0.55\textwidth}{@{\extracolsep{\fill} } ll}
		\textnormal{"All mammals are dogs"} & $P\rightarrow Q$ \\
		\textnormal{"A cat is a mammal"} & $P$ \\
		\hline
		\textnormal{"Therefore, a cat is a dog"} & $Q$ \\
	\end{tabular*}
\end{center}
\vspace{4mm}

This conclusion is a \textit{syntactic consequence} of the premises ($P\vdash Q$). That is, assuming that the premises are true, the conclusion is also true. However, a cat is not a dog. The premise that "All mammals are dogs" is, in reality, false. If we change it to something semantically true such as "All mammals are warm-blooded", we can conclude that "A cat is warm-blooded". In this case, the conclusion is a \textit{semantic consequence} of the premises ($P\vDash Q$). This syntactic-semantic difference will come up repeatedly throughout this guide. \\

Of course, arguments typically have more than one step. \\
...
% Buddhism - a faculty of the mind of immediate knowledge
...
\subsection*{The Mathematician, the Scientist, the Computer}
\addcontentsline{toc}{subsection}{The Logician and the Scientist}
...
\subsection*{A History of Tools}
\addcontentsline{toc}{subsection}{A History of Tools}
...
\subsection*{Talking with Talos}
\addcontentsline{toc}{subsection}{Talking with Talos}
...
% We only need a language now to express ourselves.
% Creation of language, humanity stuff, natural language (informal)
% Structure of natural language, types of sentences (declarative/imperative)
% Formal definition of language, mention grammar informally
% Syntactic/semantic divide

% So then... what separates Talos from man?
% Deductive computers, AI, General AI, Artificial Conciousness

%----------------------------------------

\subsection*{Computation in Layman's Terms}
\addcontentsline{toc}{subsection}{Talking with Talos}
