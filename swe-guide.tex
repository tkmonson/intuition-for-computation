...
\begin{tcolorbox}[breakable, enhanced, colback=textbook-blue, sharp corners]
	...
	For example, yes, you can tie a single knot in a shoelace, and you might then call the floppy bits on either side of it "links." But when you call something like this a link, what you are really pointing out is the \textit{finite length} of the entity. That is, the "string link" \textit{starts} at the knot and \textit{ends} at the tip of the aglet. But length is a peripheral detail for links---the essence of a \textit{link} is its \textit{connecting} of things. Thus, it is better to envision a taut string that stretches to infinity in both directions and to think of a link as a \textit{connector of a knot pair} whose length may or may not be important. With this model, there must be at least two knots in the string if we are to think of them as \textit{nodes}. \\
	...
	\begin{tikzpicture}[scale=0.2]
		...
		\draw [ultra thick] (-18.75,0) -- ++(-9,0);
		\draw [ultra thick] (18.75,0) -- ++(9,0);
	\end{tikzpicture}
	\end{center}
	
	% Clarify the differences in knot theory. Move on to graph theory.
	% Blue box for the term node. Node is a joint or a communication hub in an information system. It is connected to other nodes by links or communication channels. Node is Latin for knot. So node/connection in information science, vertex/edge in graph theory.
	% The difference between a taxonomy and an ontology is that the former has child nodes. It covers more than just is-a relations.
	\vspace{3mm}
\end{tcolorbox}
\vspace{2\baselineskip}

% Introduce abstract-concrete duality (different than fundamental/derived duality).
% Instance-of is different than is-a. Is-a is a relation that is abstract->abstract. Instance-of is concrete->abstract (instantiation is abstract->concrete).
