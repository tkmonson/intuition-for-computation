...
The journey to a deep familiarity with these terms will be an arduous one, requiring an open mind and an ample store of time set aside for contemplation.

% Descartes, Meditations

% But if we wish to build an intuition for computation from the foundations and up, we must start in the dark, knowing nothing, as free as we can hope to be of our accumulated cognitive biases. Let us then take a page from Hesiod and start our tale with the chaos of total uncertainty. \\
...
% Tabula rasa vs. pre-programming

% Give into absolute uncertainty. We will build from there.

% Chunking: trace the CST automaton of universes. Call it a map though, not an automaton.

\subsection{Things and Universes}

% SEP: Mereology
% Wiki: Gestalt psychology
...
% Order
% Hierarchy (nested hierarchy is taxonomy)
% Containment hierarchy (a taxonomy of strict sets)
% Subsumptive containment hierarchy (is-a, OOP class inheritance, general-to-specific)
